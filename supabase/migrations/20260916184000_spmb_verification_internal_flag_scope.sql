-- Scope the internal verification guard flag to the exact protected updates.
-- This prevents a trigger invoked early in a larger transaction (for example
-- akademik_save_siswa) from leaving the bypass flag enabled for later writes.
