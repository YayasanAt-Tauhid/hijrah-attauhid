...
BEGIN
  ...
  WHERE g.aktif
    AND clock_timestamp() >= g.tanggal_mulai
    AND (g.tanggal_selesai IS NULL OR clock_timestamp() < g.tanggal_selesai)
  ORDER BY g.urutan, g.tanggal_mulai
  LIMIT 1;

  IF current_wave.id IS NULL THEN
    RAISE EXCEPTION 'Pendaftaran SPMB sedang ditutup. Tidak ada gelombang aktif saat ini';
  END IF;
  ...
  SET spmb_gelombang_id = current_wave.id
  WHERE siswa_id = (saved->>'id')::uuid;
  ...
  RETURN saved || jsonb_build_object(
    ...
    'kategori', category_value,
    'jenis_pendaftaran', registration_type,
    'gelombang_id', current_wave.id,
    'gelombang_nama', current_wave.nama,
    'gratis_pendaftaran', current_wave.gratis_pendaftaran
  );
END
$$;
