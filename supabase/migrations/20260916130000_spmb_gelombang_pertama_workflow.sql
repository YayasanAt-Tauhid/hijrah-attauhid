-- SPMB Gelombang Pertama 2027/2028.
-- Menyimpan milestone seleksi secara permanen dan checklist verifikasi per nilai.
-- Gratis biaya pendaftaran hanya untuk pendaftar 21 Sep 2026 00:00 WIB
-- sampai 23 Okt 2026 23:59:59 WIB. Pendaftaran setelah itu kembali berbayar.

ALTER TABLE public.siswa_detail
  ADD COLUMN IF NOT EXISTS spmb_tanggal_tes timestamptz,
  ADD COLUMN IF NOT EXISTS spmb_tanggal_lulus timestamptz,
  ADD COLUMN IF NOT EXISTS spmb_tanggal_daftar_ulang timestamptz,
  ADD COLUMN IF NOT EXISTS spmb_verifikasi_fields jsonb NOT NULL DEFAULT '{}'::jsonb;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'siswa_detail_spmb_verifikasi_fields_object_check'
      AND conrelid = 'public.siswa_detail'::regclass
  ) THEN
    ALTER TABLE public.siswa_detail
      ADD CONSTRAINT siswa_detail_spmb_verifikasi_fields_object_check
      CHECK (jsonb_typeof(spmb_verifikasi_fields) = 'object');
  END IF;
END
$$;

COMMENT ON COLUMN public.siswa_detail.spmb_tanggal_tes IS
  'Waktu saat admin menandai calon murid sudah mengikuti tes SPMB';
COMMENT ON COLUMN public.siswa_detail.spmb_tanggal_lulus IS
  'Waktu saat admin menandai calon murid lulus seleksi SPMB';
COMMENT ON COLUMN public.siswa_detail.spmb_tanggal_daftar_ulang IS
  'Waktu saat admin menandai calon murid sudah daftar ulang';
COMMENT ON COLUMN public.siswa_detail.spmb_verifikasi_fields IS
  'Checklist verifikasi per field pada tab Data SPMB; key adalah nama field dan value boolean';
...
BEGIN
  ...
  IF jenis IS NOT NULL THEN
    ...
    WHERE p.siswa_id = s.id
      ...
      AND (
        (d.tahun_ajaran_id IS NOT NULL AND p.tahun_ajaran_id = d.tahun_ajaran_id)
  ...
  -- 24 Okt 2026 00:00 WIB = 23 Okt 2026 17:00 UTC
  ...
  -- Pendaftar Gelombang Pertama tidak diwajibkan membayar biaya pendaftaran.
