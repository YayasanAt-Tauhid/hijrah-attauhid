...
BEGIN
     ...
     AND (
       ...
       OR NOT (
         ...
         OR (
           ...
           AND public.can_manage_akademik_departemen(auth.uid(),target_dept)
         )
       )
     )
  THEN
  ...
  CASE p_action
    WHEN 'tes' THEN
      ...
      UPDATE public.siswa_detail SET spmb_tanggal_tes=marked_at WHERE siswa_id=p_siswa_id;
      RETURN marked_at;
    WHEN 'lulus' THEN
      ...
      SET spmb_status_kelulusan='lulus',spmb_tanggal_lulus=marked_at,spmb_tanggal_keputusan=marked_at
    ...
    WHEN 'tidak_lulus' THEN
      ...
      UPDATE public.siswa_detail
      SET spmb_status_kelulusan='tidak_lulus',spmb_tanggal_lulus=NULL,
          spmb_tanggal_daftar_ulang=NULL,spmb_tanggal_keputusan=marked_at
      WHERE siswa_id=p_siswa_id;
      RETURN marked_at;
    WHEN 'daftar_ulang' THEN
      ...
      UPDATE public.siswa_detail SET spmb_tanggal_daftar_ulang=marked_at WHERE siswa_id=p_siswa_id;
      RETURN marked_at;
    ELSE
...
GRANT EXECUTE ON FUNCTION public.spmb_mark_milestone(uuid,text) TO authenticated,service_role;
