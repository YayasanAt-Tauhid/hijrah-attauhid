...
describe('Integration API v1 contract', () => {
  ...
  it('exposes employee read/import behind explicit scopes without role or finance writes', () => {
    ...
    expect(api).toContain("const allowedFields=new Set(['pegawai_id','nip','nama','email','jenis_kelamin','jabatan','departemen_id','status','tanggal_masuk','tanggal_pensiun','golongan_terakhir'])")
    expect(api).toContain("const PEGAWAI_BASE='id,nip,nama,email,jenis_kelamin,jabatan,departemen_id,status,tanggal_masuk,tanggal_pensiun,golongan_terakhir,created_at'")
    ...
    expect(docs).toContain('Email pegawai tersedia')
    expect(openapi).not.toContain('Omitted unless token has pegawai:contact:read')
    expect(postman).toContain('Email is included')
  })
