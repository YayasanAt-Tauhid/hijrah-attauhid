...
const SCOPES = ['pendaftaran:read','pendaftaran:identity:read','pendaftaran:contact:read','pendaftaran:sensitive:read','pendaftaran:documents:read','siswa:read','siswa:identity:read','kelas:read','pegawai:read','pegawai:contact:read','pegawai:write','pendaftaran:milestone:update'] as const
...
function studentIdentityView(ctx:Ctx,s:any,d:any){return has(ctx,'siswa:identity:read')?{nisn:s.nisn??null,nik_hijrah:d?.nik??null,nik_dapodik:d?.nik_dapodik??null}:{}}
async function studentIdentityDetail(ctx:Ctx,siswaId:string){if(!has(ctx,'siswa:identity:read'))return null;const {data}=await (ctx.admin.from('siswa_detail') as any).select('siswa_id,nik,nik_dapodik').eq('siswa_id',siswaId).maybeSingle();return data||null}
 ...
 try{
  ...
  if(type==='pendaftaran'){
   ...
   while(items.length<=lim){
    ...
    let identityMap=new Map<string,any>()
    if(has(ctx,'siswa:identity:read')&&ids.length){const {data:identityRows,error:identityError}=await (ctx.admin.from('siswa_detail') as any).select('siswa_id,nik,nik_dapodik').in('siswa_id',ids);if(identityError)throw identityError;identityMap=new Map((identityRows||[]).map((x:any)=>[x.siswa_id,x]))}
    ...
    for(const student of rows){
     ...
     items.push({id:student.id,nis:student.nis,...studentIdentityView(ctx,student,identityMap.get(student.id)),nama:student.nama,jenis_kelamin:student.jenis_kelamin,status:student.status,unit:{id:student.departemen_id},angkatan:{id:student.angkatan_id},terverifikasi:student.terverifikasi,created_at:student.created_at})
     if(items.length>lim)break
    }
    if(rows.length<batchSize||items.length>lim)break
   }
